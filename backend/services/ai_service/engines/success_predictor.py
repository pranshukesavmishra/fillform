...
class SuccessPredictor:
    ...
    async def predict(
        ...
        # ── Marks advantage ────────────────────────────────────────────────────
        student_marks = career_dna.get("marks_percent") or career_dna.get(
            "marks_12th_percent", 60
        )
        min_marks = opportunity_data.get("eligibility_rules", {}).get(
        ...
        allowed_states = opportunity_data.get("eligibility_rules", {}).get(
            "states_allowed", ["ALL"]
        )
        ...
        doc_readiness = len([d for d in required_docs if d in available_docs]) / max(
            len(required_docs), 1
        )
        ...
        historical_count = (
            opportunity_data.get("platform_success_count", 0) + platform_applicants
        )
